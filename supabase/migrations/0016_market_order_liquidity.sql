-- ===========================================================================
--  FIX : un ordre MARKET sans liquidité créait un ordre zombie.
--
--  Problème : sur un carnet vide (pas de contrepartie), place_order créait un
--  ordre MARKET en statut OPEN avec 0 fill. L'utilisateur croyait que son achat
--  avait marché mais rien ne se passait (pas de débit, pas de position).
--
--  Un ordre MARKET doit s'exécuter IMMÉDIATEMENT ou échouer. On ajoute donc un
--  check : après la boucle de matching, si c'est un MARKET et que rien n'a été
--  rempli, on lève une erreur explicite et on ROLLBACK (l'ordre est annulé).
--
--  Les ordres LIMIT restent en attente (comportement normal d'un carnet).
-- ===========================================================================
...
declare
  ...
  v_sp_old_qty bigint;
  v_sp_old_lk  bigint;
  v_filled_total bigint := 0;   -- FIX : tracking du total rempli
begin
  ...
  while v_remaining > 0 loop
    ...
    v_filled_total := v_filled_total + v_fill_qty;
    v_remaining := v_remaining - v_fill_qty;
  end loop;

  -- ============================================================
  --  FIX : un ordre MARKET DOIT être rempli ou échouer.
  --  S'il n'y a aucune liquidité (0 fill), on rejette + rollback.
  --  Un MARKET partiellement rempli annule aussi le solde non rempli.
  -- ============================================================
  if p_order_type = 'MARKET' and v_filled_total < p_quantity then
    if v_filled_total = 0 then
      -- Rien rempli du tout → carnet vide → erreur claire.
      perform public.market_error(
        'Aucune contrepartie disponible dans le carnet pour cet ordre au marché. '
        || 'Essayez un ordre LIMIT ou attendez qu''un autre joueur place une vente.'
      );
    else
      -- Partiellement rempli : on marque FILLED sur ce qui a été fait,
      -- le reste est annulé (comportement standard des exchanges).
      update public.orders
        set status = 'FILLED'::public.order_status,
            quantity = v_filled_total   -- ajuste la quantité demandée au rempli
      where id = v_order_id;
    end if;
  else
    -- Recalcule le statut final de l'ordre (LIMIT ou MARKET entièrement rempli).
    update public.orders
      set status = case
