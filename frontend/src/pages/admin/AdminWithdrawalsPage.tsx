  ...
  return (
    <div className="space-y-4">
      <header className="mb-4">
        <h1 className="text-2xl font-black uppercase tracking-tight text-zinc-900">Retraits</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((w) => (
            <div key={w.id} className="card space-y-4 flex flex-col justify-between hover:border-zinc-300">
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2 border-b border-zinc-150 pb-3">
                  <div>
                    <span className="font-mono font-bold text-[10px] tracking-wider text-zinc-800 bg-zinc-100 border border-zinc-200 px-2 py-0.5 rounded">{w.code}</span>
                    <p className="font-bold text-sm text-zinc-900 mt-2">{w.user_phone}</p>
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px] text-zinc-500 font-semibold uppercase tracking-wider">
                  <span>Opérateur : <b className="text-zinc-800">{w.operator_label}</b></span>
                  <span>Vers : <b className="text-zinc-800">{w.recipient_phone}</b></span>
                  <span className="col-span-2 text-[10px] text-zinc-400 mt-1">Demandé : {dateFr(w.created_at)}</span>
                </div>
              </div>
                ...
                {w.status === "PENDING" && (
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="btn bg-blue-600 hover:bg-blue-700 text-white border border-blue-600 px-3 py-1.5 text-xs font-bold uppercase tracking-wider"
                    >
                      Payer
                    </button>
                  </div>
