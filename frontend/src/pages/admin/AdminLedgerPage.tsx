  ...
  return (
    <div className="space-y-4">
      <header>
        ...
        <p className="text-sm text-slate-400">Journal complet des mouvements de solde.</p>
      </header>

      <div className="flex gap-1 overflow-x-auto rounded-xl bg-ink-800 p-1">
        {FILTERS.map((f) => (
          <button
            key={f.key}
            ...
            className={cx(
              ...
              type === f.key ? "bg-brand-600 text-white" : "text-slate-400"
            )}
          >
            {f.label}
          </button>
              ...
              <div key={e.id} className="card flex items-center justify-between gap-2 py-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="truncate text-xs text-slate-400">{e.reference || "—"}</span>
                  </div>
                  <p className="mt-1 truncate text-xs text-slate-400">
                    {e.user_phone}
                    {e.note ? ` · ${e.note}` : ""}
                  </p>
                  <p className="text-[11px] text-slate-500">{dateFr(e.created_at)}</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-[11px] text-slate-500">→ {parseFloat(e.balance_after).toLocaleString("fr-FR")}</p>
                </div>
              </div>
            );
          })}
